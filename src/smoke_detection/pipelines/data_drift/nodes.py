...
def data_drift(data_reference: pd.DataFrame, data_analysis: pd.DataFrame):
    ...
    column_names=["Temperature[C]", "Humidity[%]"],
    ...
    data_drift_report.run(current_data=data_analysis[["Temperature[C]", "Humidity[%]"]] , reference_data=data_reference[["Temperature[C]", "Humidity[%]"]], column_mapping=None)
